<template>
  <div class="v-catalog-item">
    ...
    <button class="v-catalog-item_btn_card-item" @click="sendDataToParent">click</button> <!-- мы привязали наш метод с помощью клика -->
  </div>
</template>
...
export default {
  ...
  methods: { // отслеживание действий пользователя, модальные окна и т.д 
    sendDataToParent() { // мы записали метод и дали ему название, с помощью которого будет получена информация в наш родитель
    this.$emit('cardInfo', this.product_data.article) // $emit мы емитем - уточнить?, далее первым аругментом он принимает ключ, что примет родитель, там мы
